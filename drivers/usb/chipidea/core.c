...
#define HW_USBCTRL_PORTSC1_PHCD		(1 << 23)
...
static int ci_hdrc_suspend(struct device *dev)
{
	...
	hw_write(ci, OP_PORTSC, HW_USBCTRL_PORTSC1_PHCD,
		 HW_USBCTRL_PORTSC1_PHCD);
	usb_phy_set_suspend(ci->platdata->phy, 1);
	return 0;
}
...
static int ci_hdrc_resume(struct device *dev)
{
	...
	hw_write(ci, OP_PORTSC, HW_USBCTRL_PORTSC1_PHCD,
		 ~HW_USBCTRL_PORTSC1_PHCD);
	usb_phy_set_suspend(ci->platdata->phy, 0);
	return 0;
}
